...
import { projectShortName, proposeAgents } from "../lib/propose-agents.mjs";
...
test("proposeAgents on vue3 project suggests frontend + qa + domain + pdf when applicable", () => {
	...
	assert.ok(
		slugs.some((s) => s.includes("frontend-vue-specialist")),
		"frontend-vue suggested",
	);
	assert.ok(
		slugs.some((s) => s.includes("qa-engineer")),
		"qa suggested",
	);
	assert.ok(
		slugs.some((s) => s.includes("domain-expert")),
		"domain suggested",
	);
	assert.ok(
		slugs.some((s) => s.includes("pdf-rendering-specialist")),
		"pdf suggested",
	);
});
