...
const baseConfig = () => ({
	...
	branches: {
		integration: "dev",
		release: "main",
		patterns: { feature: "f", fix: "x", chore: "c", scratch: "s" },
	},
	roles: [
		{ name: "po", agent: "bundled", capabilities: { can_write: false } },
		{
			name: "backend",
			agent: "bundled",
			capabilities: { can_write: true, scope: ["**"] },
		},
	],
	pipeline: [
...
test("shouldSkipStage skips per tier.skip_stages", () => {
	...
	const stage = {
		stage: "refine",
		owner: ["po"],
		optional_when: "tier == 'minimal'",
	};
	assert.equal(
		engine.shouldSkipStage(stage, { tier: "minimal", epic: { size: "small" } }),
		true,
	);
	assert.equal(
		engine.shouldSkipStage(stage, { tier: "full", epic: { size: "large" } }),
		false,
	);
	rmSync(dir, { recursive: true, force: true });
});
...
test("runEpic walks pipeline and dispatches in order", async () => {
	...
	const r = await engine.runEpic({
		epic: { id: "E-01", size: "large" },
...
test("parallelism: full dispatches owners simultaneously", async () => {
	...
	const r = await engine.runEpic({
		epic: { id: "E-03", size: "large" },
		integrationBranch: "dev",
	});
	assert.equal(r.ok, true);
	// Both owners called
	const buildCalls = calls
		.filter((c) => c.stage === "build")
		.map((c) => c.role)
		.sort();
