...
test("commit-format: passes on conventional-commit messages", async () => {
  ...
  execSync('git add -A && git commit -q -m "feat(scope): real change"', { cwd: dir });
  const r = await commitFormat({ projectRoot: dir, config: { base_ref: "HEAD~1", require_scope: true } });
  assert.equal(r.status, "pass");
  rmSync(dir, { recursive: true, force: true });
});
...
test("commit-format: blocks on missing scope when require_scope is true", async () => {
  const dir = makeRepo();
  writeFileSync(join(dir, "a"), "x");
  execSync('git add -A && git commit -q -m "feat: missing scope"', { cwd: dir });
  const r = await commitFormat({ projectRoot: dir, config: { base_ref: "HEAD~1", require_scope: true } });
  assert.equal(r.status, "block");
  rmSync(dir, { recursive: true, force: true });
});
...
test("commit-format: passes when require_scope is false", async () => {
  ...
  const r = await commitFormat({ projectRoot: dir, config: { base_ref: "HEAD~1", require_scope: false } });
  assert.equal(r.status, "pass");
  rmSync(dir, { recursive: true, force: true });
});
...
test("commit-format: blocks on unknown commit type", async () => {
  const dir = makeRepo();
  writeFileSync(join(dir, "a"), "x");
  execSync('git add -A && git commit -q -m "weird(scope): wrong type"', { cwd: dir });
  const r = await commitFormat({ projectRoot: dir, config: { base_ref: "HEAD~1" } });
  assert.equal(r.status, "block");
  rmSync(dir, { recursive: true, force: true });
});
