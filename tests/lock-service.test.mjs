...
test("markdown lock: second claim on same epic by different branch is rejected", async () => {
  const dir = makeRepo();
  const lock = await LockService.create({ projectRoot: dir, lockConfig: { kind: "markdown" } });
  await lock.claim({ branch: "feature/e-01", epic: "E-01", stage: "build", tier: "full" });
  const r = await lock.claim({ branch: "feature/e-01-fix", epic: "E-01", stage: "build", tier: "full" });
...
test("markdown lock: re-claim on same branch updates fields", async () => {
  const dir = makeRepo();
  const lock = await LockService.create({ projectRoot: dir, lockConfig: { kind: "markdown" } });
  await lock.claim({ branch: "feature/e-02", epic: "E-02", stage: "refine", tier: "full" });
  await lock.claim({
    branch: "feature/e-02",
    epic: "E-02",
    stage: "build",
...
test("markdown lock: update mutates one row", async () => {
  const dir = makeRepo();
  const lock = await LockService.create({ projectRoot: dir, lockConfig: { kind: "markdown" } });
  await lock.claim({ branch: "feature/e-03", epic: "E-03", stage: "refine", tier: "light" });
  const r = await lock.update({ branch: "feature/e-03", fields: { stage: "ship" } });
  assert.equal(r.ok, true);
  const rows = await lock.list();
...
test("markdown lock: release removes the row", async () => {
  const dir = makeRepo();
  const lock = await LockService.create({ projectRoot: dir, lockConfig: { kind: "markdown" } });
  await lock.claim({ branch: "feature/e-04", epic: "E-04", stage: "ship", tier: "full" });
...
test("markdown lock: stale detection flags branches with no recent commits", async () => {
  ...
  await lock.claim({ branch: "feature/e-06", epic: "E-06", stage: "build", tier: "full" });
  const rows = await lock.list();
  ...
  await lock2.claim({ branch: "feature/e-07", epic: "E-07", stage: "build", tier: "full" });
