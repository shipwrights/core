...
test("transitionPhase: running_epic → awaiting_merge is allowed", () => {
  ...
  try {
    ...
    state = transitionPhase(state, "awaiting_merge", { current: { ...state.current, pr: 9 } }, { cwd });
...
test("markCurrentCompleted bumps counter + moves to between_epics", () => {
  ...
  try {
    ...
    state = transitionPhase(state, "running_epic", { current: { ticket: "NVC-1" } }, { cwd });
    state = transitionPhase(state, "awaiting_merge", { current: { ...state.current, pr: 1 } }, { cwd });
...
test("markCurrentCompleted in unbounded loop never moves to done", () => {
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      state = transitionPhase(state, "running_epic", { current: { ticket: `NVC-${i}` } }, { cwd });
      state = transitionPhase(state, "awaiting_merge", { current: state.current }, { cwd });
      state = markCurrentCompleted(state, { cwd });
    }
...
test("markCurrentCompleted throws when there's no current ticket", () => {
  ...
  try {
    const state = startLoop({ cwd });
    assert.throws(() => markCurrentCompleted(state, { cwd }), /no current ticket/);
  } finally {
    rmSync(cwd, { recursive: true, force: true });
  }
});
