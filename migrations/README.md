...
Each file here transforms a `.shipwright.yml` from one schema version to the next. They run automatically on `/shipwright:upgrade`.
...
Without migrations, every breaking schema change taxes every consumer. With them, `/shipwright:upgrade` is just `npm update && /shipwright:upgrade`. The plugin can evolve.
