...
export function createSource({
	...
	function listIssues(filter = {}) {
		const labelArg = (filter.labels ?? labels)
			.map((l) => `--label "${l}"`)
			.join(" ");
		const out = gh(
	...
	return {
		async healthcheck() {
			try {
				...
				throw new Error(
					`Cannot access repo ${repo} via gh CLI: ${err.message}`,
				);
			}
		...
		async markStatus(itemId, status) {
			...
			try {
				...
				const target = issues.find(
					(i) => epicIdFromIssue(i, idPrefix) === itemId,
				);
				if (!target) return;
				gh(
					`issue edit ${target.number} --repo ${repo} --add-label "status:${status}"`,
					projectRoot,
				);
			} catch (err) {
...
function extractLabel(issue, prefix) {
	return issue.labels
		?.find((l) => l.name.startsWith(prefix))
