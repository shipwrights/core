...
export function createSource({ projectRoot, state_dir = "docs/backlog/epics" } = {}) {
  ...
  return {
    ...
    async listAvailable(filter = {}) {
      ...
      const statuses = filter.statuses ?? ["idea", "refined", "designed", "planned", "built", "tested", "reviewed"];
    ...
    async materialize(item, targetDir) {
      ...
      const expectedPath = item.metadata?.sourcePath ?? join(dir, `${item.id}-${slugify(item.title)}.md`);
    ...
    async markStatus(itemId, status) {
      ...
      if (!target) return;
      const updated = target.content.replace(/^status:\s*\S+/m, `status: ${status}`);
      if (updated !== target.content) writeFileSync(target.path, updated, "utf8");
    },
    ...
    async attachPR(itemId, prUrl) {
      ...
      if (/^## Related PRs/m.test(updated)) {
        ...
        updated = updated.replace(/(## Related PRs[\s\S]*?)(?=\n## |\s*$)/, (m) => `${m.replace(/\s+$/, "")}\n${link}\n`);
      } else {
