...
const featureTip = execSync(`git rev-parse ${featureBranch}`, { encoding: "utf8" }).trim();
...
if (ff.status !== 0) {
  console.error("fast-forward merge failed; the scratch is likely behind. Re-rebase the scratch.");
  process.exit(1);
}
...
if (config.scratch?.cleanup_on_integrate) {
  ...
  if (config.scratch?.push_to_remote) {
    spawnSync("git", ["push", "origin", "--delete", scratchBranch], { stdio: "inherit" });
  }
}
