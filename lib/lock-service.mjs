...
class MarkdownLock {
	...
	async claim({ branch, epic, stage, tier, specialists = [] }) {
		...
		if (existing && existing.branch !== branch) {
			return {
				ok: false,
				reason: `epic ${epic} already in flight on ${existing.branch}`,
				existing,
			};
		}
...
class GithubIssuesLock {
	...
	async list() {
		...
		return issues.map((i) => {
			...
			const stage =
				labels.find((l) => l.startsWith("stage:"))?.slice(6) ?? "unknown";
			const tier =
				labels.find((l) => l.startsWith("tier:"))?.slice(5) ?? "unknown";
			return {
				...
				tier,
				specialists: labels
					.filter((l) => l.startsWith("specialist:"))
					.map((l) => l.slice(11)),
...
export class LockService {
	static async create({ projectRoot, lockConfig = {} }) {
		...
		if (kind === "custom") {
			...
			if (!pkg)
				throw new Error("lock.kind: custom requires lock.config.package");
