...
//
// `/shipwrights-upgrade` calls runMigrations(config, fromVersion, toVersion) and
...
export async function runMigrations(config, fromVersion, toVersion) {
	...
	for (const step of chain) {
		const mod = await importFromPath(step.path);
		if (typeof mod.up !== "function") {
			throw new Error(
				`Migration ${step.from}-to-${step.to} missing up() export`,
			);
		}
