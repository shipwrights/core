...
export function discoverVerifications(projectRoot) {
	...
	if (existsSync(pkgPath)) {
		...
		if (pkg?.scripts) {
			...
			if (scripts.typecheck)
				recipe.typecheck = nodeRunCommand(runner, "typecheck");
			else if (scripts["type-check"])
				recipe.typecheck = nodeRunCommand(runner, "type-check");
	...
	if (!recipe.verify) {
		recipe.notes.push(
			"no verify command auto-detected; supply one in .shipwrights.yml verify.overrides",
		);
	}
