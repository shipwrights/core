...
function warn(label, reason) {
	checks.push({ status: "warn", label, reason });
}
function fail(label, reason) {
	checks.push({ status: "fail", label, reason });
}
...
export async function runDoctor({ projectRoot, pluginRoot }) {
	...
	for (const role of config.roles) {
		try {
			...
			if (resolved.kind !== "npm" && resolved.content) {
				const tools =
					(resolved.content.match(/^tools:\s*\[([^\]]+)\]/m) ?? [])[1] ?? "";
				const hasWrite = /\bWrite\b|\bEdit\b/.test(tools);
				if (writeRequested && !hasWrite) {
					warn(
						`Role ${role.name}`,
						`agent has no Write/Edit tools but role requests can_write: true`,
					);
					continue;
				}
				if (
					!writeRequested &&
					/\bWrite\b/.test(tools) &&
					role.capabilities?.can_write === false
				) {
					warn(
						`Role ${role.name}`,
						`agent has Write but role declares can_write: false`,
					);
					continue;
				}
			}
	...
	const verifyCmd =
		config.verify?.overrides?.verify ?? config.verify?.detected?.verify;
	if (verifyCmd) {
	...
	const tlmPath = join(
		projectRoot,
		config.telemetry?.log_path ?? ".shipwrights/telemetry.jsonl",
	);
	ok(`Telemetry: ${tlmPath}`);

	const requiredLabels = (config.merge?.auto_merge_labels ?? []).concat(
		[config.merge?.block_label].filter(Boolean),
	);
	if (requiredLabels.length > 0) {
		try {
			...
			if (missing.length === 0)
				ok(`GitHub labels: all ${requiredLabels.length} present`);
	...
	if (shouldCheckBotToken(config)) {
		try {
			const out = execSync("gh secret list --json name", {
				cwd: projectRoot,
				encoding: "utf8",
			});
			const existing = new Set(JSON.parse(out).map((s) => s.name));
			if (existing.has("SHIPWRIGHTS_BOT_TOKEN")) {
				ok("GitHub secret: SHIPWRIGHTS_BOT_TOKEN configured");
			} else {
				warn(
					"GitHub secret: SHIPWRIGHTS_BOT_TOKEN",
					"missing; bot-created follow-up PRs may not trigger protected-branch checks",
				);
			}
		} catch {
			warn(
				"GitHub secret: SHIPWRIGHTS_BOT_TOKEN",
				"could not query (gh CLI not available)",
			);
		}
	}
...
function shouldCheckBotToken(config) {
	const autoMergeLabels = config.merge?.auto_merge_labels ?? [];
	const anyTierAutoMerges = Object.values(config.tiers ?? {}).some(
		(tier) => tier?.auto_merge === true,
	);
	return autoMergeLabels.length > 0 || anyTierAutoMerges;
}
