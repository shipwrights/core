...
function defaultPipelineForRoles(roles) {
  ...
  const stages = [
    { stage: "refine", owner: ["po"], optional_when: "tier == 'trivial' || tier == 'minimal'" },
    {
      stage: "slice",
      owner: ["orchestrator"],
    ...
    },
    {
      ...
      owner: buildOwners.length ? buildOwners : ["backend"],
      parallelism: "full",
    ...
    },
    {
      stage: "integrate",
      owner: ["orchestrator"],
      integrate_order: buildOwners.length ? buildOwners : ["backend"],
  ...
  if (roles.some((r) => r.name === "browser")) {
...
export async function runInit({ projectRoot, pluginRoot, args = [] }) {
  ...
  const yamlText = `# Shipwright config — see https://github.com/dacostaaboagye/shipwright\n${toYaml(config)}`;
  ...
  const commitMsg = `chore: install @shipwright/core v${pkgVersion}`;
...
function templateDestination(rel, projectRoot) {
  ...
  // templates/docs/** -> docs/**
  const normalized = rel.replace(/^github\//, ".github/");
