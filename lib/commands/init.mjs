// `shipwrights init` — non-destructive scaffold.
//
...
async function ask(question, defaultValue = "") {
	...
	const ans = await new Promise((res) =>
		r.question(`${question} [${defaultValue}] `, res),
	);
	r.close();
...
function defaultPipelineForRoles(roles) {
	...
	const designAndBuildOwners =
		buildOwners.length > 0 ? buildOwners : ["orchestrator"];
...
function buildDefaultConfig({ projectRoot, recipe, layout, projectName }) {
	...
	return {
		...
		merge: {
			...
			stack_depth: 3,
			auto_merge_labels: [
				DEFAULT_LABELS.trivial.name,
				DEFAULT_LABELS.minimal.name,
			],
		...
		telemetry: {
			enabled: true,
			log_path: ".shipwrights/telemetry.jsonl",
...
async function resolveConflict(rel, { nonInteractive, force }) {
	...
	console.log(`\nConflict: ${rel}`);
	const ans = (
		await ask("(s)kip / (o)verwrite / (e)xample / (v)iew-diff", "s")
	).toLowerCase();
...
export async function runInit({ projectRoot, pluginRoot, args = [] }) {
	...
	if (!gitClean(projectRoot) && !flags.force) {
		console.error(
			"Working tree is dirty. Commit or stash, then re-run (or pass --force).",
		);
		process.exit(2);
	}
	...
	if (existsSync(configPath)) {
		console.error(
			".shipwrights.yml already exists. Use /shipwrights-upgrade to bump.",
		);
		process.exit(2);
	}
	...
	if (!flags.nonInteractive) {
		config.branches.integration = await ask(
			"Integration branch?",
			config.branches.integration,
		);
		config.branches.release = await ask(
			"Release branch?",
		...
		rolesByName[r.name] = {
			agent:
				typeof r.agent === "string"
					? r.agent
					: (r.agent?.user ?? r.agent?.custom ?? "bundled"),
		};
	}
	...
	for (const tmpl of allTemplates) {
		...
		const cl = classifyTemplate({
			srcAbs: tmpl.absolute,
			destAbs: dest,
	...
	// Add the .shipwrights.yml itself.
	...
	for (const p of plan) {
		if (
			...
			p.action === "skip-conflict" ||
			p.action === "skip-protected"
		)
			continue;
	...
	// The scratch-branch lifecycle scripts (templates/scripts/shipwrights/*.mjs)
	...
	// Record hashes of managed template files so the first /shipwrights-upgrade
	// after this init has a baseline to compare against — without it, every
	// managed file looks "preexisting" and gets skipped.
	const manifest = readManifest(projectRoot);
	for (const rel of MANAGED_TEMPLATE_RELS) {
		const dest = templateDestPath(rel, projectRoot);
		if (!existsSync(dest)) continue;
		manifest.files[rel] = hashContent(readFileSync(dest, "utf8"));
	}
	writeManifest(projectRoot, manifest);

	// installed.json marker for /shipwrights-upgrade
	const installedDir = join(projectRoot, ".shipwrights");
	mkdirSync(installedDir, { recursive: true });
	writeFileSync(
		join(installedDir, "installed.json"),
		JSON.stringify(
			{ version: config.version, installedAt: new Date().toISOString() },
			null,
			2,
		),
		"utf8",
	...
	// Wire bundled skills + agents into .claude/ so Claude Code discovers
	// them without the consumer running cp/ln manually.
	...
	console.log(`\n✓ Shipwrights installed at v${pkgVersion}`);
	console.log(
		"✓ Bundled skills + agents wired to .claude/skills/shipwrights/ and .claude/agents/shipwrights/",
	);
	console.log("✓ Undo with `git revert HEAD`");
	console.log("\nNext:");
	console.log(
		"  pnpm install                          # install yaml + minimatch (added to devDependencies)",
	);
	console.log(
		"  /shipwrights-doctor                   # validate (restart Claude Code to pick up new skills)",
	);
	...
	console.log(
		"  /shipwrights-epic <id>                # drive your first epic",
	);
}
