...
export async function runInit({ projectRoot, pluginRoot, args = [] }) {
  ...
  if (existsSync(configPath)) {
    console.error(".shipwrights.yml already exists. Use /shipwrights-upgrade to bump.");
    process.exit(2);
  }
  ...
  plan.push({ rel: ".shipwrights.yml", dest: configPath, action: "write", content: yamlText });
  ...
  console.log("✓ Bundled skills + agents wired to .claude/skills/shipwrights/ and .claude/agents/shipwrights/");
  console.log("✓ Undo with `git revert HEAD`");
  console.log("\nNext:");
  console.log("  pnpm install                          # install yaml + minimatch (added to devDependencies)");
  console.log("  /shipwrights-doctor                   # validate (restart Claude Code to pick up new skills)");
  ...
  console.log("  /shipwrights-epic <id>                # drive your first epic");
}
