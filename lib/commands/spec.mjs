...
export async function runSpec({ projectRoot, pluginRoot, args }) {
  ...
  if (!flags.taskDescription) {
    console.error("usage: shipwrights spec [--auto] [--context-depth shallow|medium|deep] [--output-dir <path>] <task description>");
    process.exit(2);
  }

  if (!inGit(projectRoot)) {
    console.error("Not a git repository. Initialize one first or change directory.");
    process.exit(2);
  }
  if (!isClean(projectRoot)) {
    console.error("Working tree is dirty. Commit or stash before drafting a spec.");
    process.exit(2);
  }
  ...
  const outputDir = flags.outputDir ?? specConfig.output_dir ?? "docs/backlog/specs";
  const contextDepth = flags.contextDepth ?? specConfig.context_depth ?? "medium";
  const approvalRequired = flags.auto ? false : (specConfig.approval_required !== false);
  ...
  console.log(`  The orchestrator skill picks up the input, runs the pipeline,`);
