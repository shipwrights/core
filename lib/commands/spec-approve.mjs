...
export async function runSpecApprove({ projectRoot, args }) {
  ...
  if (!manifest) {
    if (enforcement === "strict") {
      console.error(`Manifest not found at .shipwrights/specs/${id}.manifest.json. Cannot validate citations.`);
      console.error(`To proceed anyway, set spec.enforcement: loose in .shipwrights.yml.`);
      process.exit(2);
    }
    console.warn(`[shipwrights] manifest not found; loose mode — skipping citation check.`);
  } else {
    const result = validateCitations(plan.body, manifest);
    if (!result.ok && enforcement === "strict") {
      console.error(`Citation enforcement failed: ${result.violations.length} uncited claim(s):`);
      for (const v of result.violations) {
        console.error(`  ${v.section}: ${v.path}${v.range ? `:${v.range}` : ""}`);
      }
      console.error(`Either revise the plan or set spec.enforcement: loose to bypass.`);
      process.exit(2);
    }
    if (!result.ok) {
      console.warn(`[shipwrights] ${result.violations.length} uncited claim(s) (loose mode):`);
      for (const v of result.violations) {
        console.warn(`  ${v.section}: ${v.path}${v.range ? `:${v.range}` : ""}`);
      }
    }
  }
  ...
  spawnSync(
    "git",
    [
      "-c", "user.name=Shipwrights Approve",
  ...
  console.log(`\n✓ Spec ${id} approved.`);
  console.log(`Next: run /shipwrights-spec-build ${id} in Claude Code (or set --auto on the original draft).`);
}
