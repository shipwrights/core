...
export function installPluginSurfaces({ projectRoot, pluginRoot }) {
  ...
  writeFileSync(join(skillsDestRoot, "SHIPWRIGHTS-MANAGED.md"), MANAGED_README, "utf8");
  installed.push(join(".claude", "skills", "SHIPWRIGHTS-MANAGED.md"));

  // 4) Copy each agent file flat into .claude/agents/, but only if a
  // user-global agent of the same name doesn't already exist.
  ...
  for (const file of readdirSync(agentsSrc)) {
    ...
    const userGlobal = join(userAgentsDir, file);
    if (existsSync(userGlobal)) {
      // User has a global version — don't shadow it.
      continue;
    }
