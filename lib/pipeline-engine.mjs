...
export class PipelineEngine {
  ...
  shouldSkipStage(stage, ctx) {
    if (stage.optional_when) {
      try {
        ...
        throw new Error(`Stage ${stage.stage} optional_when invalid: ${err.message}`);
      }
    }
  ...
  rolesForStage(stage) {
    ...
    return owners.map((name) => {
      if (name === "orchestrator") return { name: "orchestrator", isOrchestrator: true };
      const role = this.config.roles.find((r) => r.name === name);
      if (!role) throw new Error(`Stage ${stage.stage} owner "${name}" not in roles`);
      return role;
    });
  }
