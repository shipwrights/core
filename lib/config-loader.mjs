...
const SCHEMA_PATH = new URL("../schemas/shipwrights-config.schema.json", import.meta.url);
let _validator = null;
...
export function loadConfig(projectRoot, configPath) {
  ...
  let fullPath;
  if (configPath) {
    fullPath = isAbsolute(configPath) ? configPath : resolve(projectRoot, configPath);
  } else {
