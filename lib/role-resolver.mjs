...
function pluginAgentPath(name) {
	return new URL(`${name}.md`, PLUGIN_AGENTS_DIR).pathname.replace(
		/^\/(\w):/,
		"$1:",
	);
}
...
export function resolveRole(role, { projectRoot }) {
	...
	if (ref?.user) {
		...
		if (!existsSync(userPath)) {
			throw new Error(
				`User-global agent "${ref.user}" not found at ${userPath}`,
			);
		}
		return {
			kind: "user",
	...
	if (ref?.custom) {
		const customPath = isAbsolute(ref.custom)
			? ref.custom
			: resolve(projectRoot, ref.custom);
		if (!existsSync(customPath)) {
			throw new Error(
				`Custom agent for role "${role.name}" not found at ${customPath}`,
			);
		}
		return {
			kind: "custom",
	...
	throw new Error(
		`Unrecognized agent ref shape for role "${role.name}": ${JSON.stringify(ref)}`,
	);
}
